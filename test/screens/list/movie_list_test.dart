...
@GenerateNiceMocks([MockSpec<MovieListBloc>()])
void main() {
  ...
  setUpAll(() {
    // Mockito needs it
    provideDummy<MovieListState>(const MovieListStateLoading());
  });

  group('MovieList', () {
    testWidgets('has an initial loading state', (tester) async {
      // Arrange
      when(mockMovieListBloc.state).thenReturn(const MovieListStateLoading());
      const widget = ListScreen();
    ...
    testWidgets('has an error state', (tester) async {
      // Arrange
      when(mockMovieListBloc.state).thenReturn(const MovieListStateError());
      const widget = ListScreen();
